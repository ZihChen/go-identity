...
MYSQL_DATABASE="ms_fatidentitycat"
ATLAS_ENV="gorm"
...
apply_migration() {
  ...
  CMD="$ATLASGO schema apply --env \"$ATLAS_ENV\""
  echo -e "👉 \033[1;33mExecuting:\033[0m $CMD"
  eval $CMD
...
schema_inspect() {
  CMD="$ATLASGO schema inspect -u \"mysql://${MYSQL_ROOT_USER}:${MYSQL_ROOT_PASSWORD}@${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DATABASE}?tls=true\""
  echo -e "👉 \033[1;33mExecuting:\033[0m $CMD"
  eval $CMD
}

migrate_status() {
  CMD="$ATLASGO migrate status -u \"mysql://${MYSQL_ROOT_USER}:${MYSQL_ROOT_PASSWORD}@${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DATABASE}?tls=true\""
  echo -e "👉 \033[1;33mExecuting:\033[0m $CMD"
  eval $CMD
}
...
compare_diff() {
  ...
  CMD="$ATLASGO schema diff \
    ...
    --to \"file://migrations\" \
    --dev-url \"mysql://REDACTED_DB_USER_DEV:REDACTED_DB_PASSWORD_DEV@REDACTED_DB_HOST:3306/ms_fatidentitycat?tls=true\""
...
case "$1" in
  ...
  diff)
    ...
    ;;
  *)
    echo "❗ Usage: bash $0 [init|apply|gen|inspect|status|diff]"
    exit 1
    ;;
esac