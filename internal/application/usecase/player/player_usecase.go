...
func (u *PlayerUseCase) PlayerLogin(
	...
	if account != nil {
		spanAttrs = append(spanAttrs, attribute.String("player.account", *account))
	}
	if playerGlobalID != nil {
		spanAttrs = append(spanAttrs, attribute.String("player.global_id", *playerGlobalID))
	}
	u.tracing.RecordSpanAttributes(span, spanAttrs...)

	// 驗證至少提供一個查詢條件
	if account == nil && playerGlobalID == nil {
		err := errors.New("either account or player_global_id must be provided")
		u.tracing.RecordSpanError(span, err)
		return "", err
	}
	...
	if playerGlobalID != nil && *playerGlobalID != "" {
		// 優先使用 playerGlobalID 查詢
		player, err = u.playerRepo.FindByGlobalID(ctx, *playerGlobalID)
	} else if account != nil && *account != "" {
		// 使用 account 和 merchantID 查詢
		player, err = u.playerRepo.FindByAccountAndMerchantID(ctx, *account, merchantID)
	} else {
		err = errors.New("invalid query parameters: empty values provided")
		u.tracing.RecordSpanError(span, err)
		return "", err
	}
	...
	if err != nil {
		if errors.Is(err, errmsg.ErrRepoPlayerNotFound) {
			u.logger.InfoLog("Player not found for login attempt",
				...
				u.logger.String("account", stringPtrToValue(account)),
				u.logger.String("global_id", stringPtrToValue(playerGlobalID)))
			u.tracing.RecordSpanError(span, err)
			return "", err
		}
		u.tracing.RecordSpanError(span, err)
	...
	// 生成JWT token
	token, err := u.jwtService.GenerateToken(player.GetAccount(), player.GetGlobalPlayerID())
	if err != nil {
...
// stringPtrToValue 輔助函數：將字串指針轉換為字串值（用於日誌）
